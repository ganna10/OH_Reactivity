#!/usr/bin/env perl
# Allocate NO3 reactivity to emitted species, only organic
# Version 0: Jane Coates 6/10/2014
...
$R->run(q` plot = ggplot(data = data, aes(x = Time, y = Reactivity, fill = Process)) `, #plot data
        ...
        q` plot = plot + scale_y_continuous(limits = c(0, 9), breaks = seq(0, 9, 2)) `,
        q` plot = plot + ggtitle("Allocated NO3 Reactivity to only VOCs") `,
...
$R->run(q` CairoPDF(file = "NO3_reactivity_allocation_organic.pdf", width = 20, height = 14) `, #save plot to file
        q` print(plot) `,
        q` dev.off() `,
);
