#!/usr/bin/env perl
# Allocate O3 reactivity to emitted species, only organic
...
$R->run(q` plot = ggplot(data = data, aes(x = Time, y = Reactivity, fill = Process)) `, #plot data
        ...
        q` plot = plot + ggtitle("Allocated O3 Reactivity to only VOCs") `,
...
$R->run(q` CairoPDF(file = "O3_reactivity_allocation_organic.pdf", width = 20, height = 14) `, #save plot to file
        q` print(plot) `,
        q` dev.off() `,
);
