#!/usr/bin/env perl
# Allocate reactivity to emitted species, only organics
...
$R->run(q` plot = ggplot(data = data, aes(x = Time, y = Reactivity, fill = Process)) `, #plot data
        ...
        q` plot = plot + ggtitle("Allocated OH Reactivity only to VOCs") `,
...
$R->run(q` CairoPDF(file = "OH_reactivity_allocation_time_series_organic_only.pdf", width = 20, height = 14) `, #save plot to file
        q` print(plot) `,
        q` dev.off() `,
);
